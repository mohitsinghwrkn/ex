...
router.route('/dEmployee/:id').get(function (req, res) {
  employeeModel.findByIdAndRemove({ _id: req.params.id }).then((employee) => {
...
const storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    const uniqueSuffix = new Date().toLocaleDateString("de-DE").replaceAll(".", "-") + " " + new Date().toLocaleTimeString().replaceAll(":", ".") + '.png'
